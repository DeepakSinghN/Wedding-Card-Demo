...
export default function EventDetails() {
  ...
  useGSAP(
    () => {
      ...
      tl.set(items.slice(1), { opacity: 0 }, 0);
      tl.set(items[0], { opacity: 1 }, 0);
      ...
      items.forEach((item, i) => {
        ...
        // Opacity-Only Crossfades (No Slide or Scale Motion) for 100% lag-free mobile transitions
        tl.to(items[i - 1], { opacity: 0, duration: transitionDuration, ease: "power2.inOut" }, segmentStart);
        tl.to(item, { opacity: 1, duration: transitionDuration, ease: "power2.inOut" }, segmentStart);
      });
